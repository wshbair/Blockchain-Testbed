...
vector<string> EVMDB::poll_tx(int block_number) {
   return poll_txs_by_block_number(endpoint_, intToHexString(block_number));
}

string EVMDB::intToHexString(int intValue) {
   string hexStr;
   std::stringstream sstream;
   sstream << "0x"
            << std::setfill ('0') << std::setw(2)  << std::hex << (int)intValue;
   hexStr= sstream.str();
   sstream.clear();
   return hexStr;
}
